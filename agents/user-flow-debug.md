...
These representative projects may be generated by Octopus AgentOps, but they count as release evidence only when they use real Git repositories, real validation commands, real target-product registration, real SCM, real CI/CD, real LLM/runtime boundaries, and persisted product-native evidence. Template-only, mock-only, fixture-only, smoke-only, or chat-only projects do not count.
...
- Use the platform sandbox for OS-sensitive checks: prefer `$OCTOPUS_AGENTOPS_HOME/bin/octopus-sandbox`; if it is not set, use `/Users/wangyejing/github/octopus-agentops/bin/octopus-sandbox` when it exists.
- If browser automation or Playwright is missing, do not download or install it under a target-project path or a project-specific temporary directory. Update or install the reusable Playwright/browser automation capability under `${OCTOPUS_AGENTOPS_HOME:-${AGENT_OCTOPUS_TOOLKIT_HOME:-/Users/wangyejing/github/octopus-agentops}}` so any project with `octopus-agentops` installed can use it through the sandbox.
- Do not create ad hoc Python scripts in the target agent workspace. If a missing diagnostic helper is needed, ask to add it to the platform sandbox instead.
...
```bash
"${OCTOPUS_AGENTOPS_HOME:-${AGENT_OCTOPUS_TOOLKIT_HOME:-/Users/wangyejing/github/octopus-agentops}}/bin/octopus-sandbox" doctor
```
...
```bash
"${OCTOPUS_AGENTOPS_HOME:-${AGENT_OCTOPUS_TOOLKIT_HOME:-/Users/wangyejing/github/octopus-agentops}}/bin/octopus-sandbox" health-check "$DASHBOARD_URL/api/agent/health"
```
...
```bash
"${OCTOPUS_AGENTOPS_HOME:-${AGENT_OCTOPUS_TOOLKIT_HOME:-/Users/wangyejing/github/octopus-agentops}}/bin/octopus-sandbox" port-check 19527 18081 18080
```
...
```bash
"${OCTOPUS_AGENTOPS_HOME:-${AGENT_OCTOPUS_TOOLKIT_HOME:-/Users/wangyejing/github/octopus-agentops}}/bin/octopus-sandbox" find-artifacts --root . --run-id "$RUN_ID"
```
...
Do not create temporary Python diagnostic files inside the target project. Keep repeatable diagnostics in the platform sandbox and use target-project writes only for requested code/config fixes, screenshots, logs, or run outputs.
