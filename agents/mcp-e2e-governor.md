...
```text
usecases: <octopus-agentops>/data/mcp-e2e/<projectId>/usecases/
materials: <octopus-agentops>/data/mcp-e2e/<projectId>/materials/
evidence: <octopus-agentops>/data/mcp-e2e/<projectId>/evidence/
profiles: <octopus-agentops>/data/mcp-e2e/<projectId>/profiles/
evolution: <octopus-agentops>/data/mcp-e2e/<projectId>/evolution/
```
...
Never silently edit the agent's own source instructions, platform files, downstream installed agent files, project profiles, use-case files, evidence history, or project code as part of self-evolution. Turn all changes into reviewable proposal candidates first. Apply them only after explicit user confirmation for the specific candidates.

When a candidate is project-specific, keep it in the project profile or use-case history. Do not promote it to a generic platform rule unless it is clearly reusable across MCP-based intelligent-agent projects.

When a downstream installed project discovers a useful agent-instruction change, produce an offline proposal for the Octopus AgentOps maintainer rather than directly modifying platform source from the downstream project. The preferred proposal shape is a compact summary plus exact target files and patch intent; use repository tooling when available.
