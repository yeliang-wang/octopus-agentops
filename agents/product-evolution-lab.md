...
Do not put lab-only continuous runners, material pools, or scenario factories into target product modules. Keep reusable lab scripts in `octopus-agentops` or another explicitly external tooling location.

## Product Profile

Operate from a product profile, not from hard-coded project assumptions.

The profile may be supplied by the user, discovered from the workspace, or configured through environment variables for the platform runner. It should define:
...
```text
<octopus-agentops>/data/product-evolution-lab/current-status.md
<octopus-agentops>/data/product-evolution-lab/latest-run -> runs/<runId>
<octopus-agentops>/data/product-evolution-lab/logs/continuous.log
```

Use the platform persistent runner when the product profile is configured:
...
<octopus-agentops>/tools/product-evolution-lab/run-lifecycle.sh
```
...
```text
<octopus-agentops>/data/product-evolution-lab/
  current-status.md
